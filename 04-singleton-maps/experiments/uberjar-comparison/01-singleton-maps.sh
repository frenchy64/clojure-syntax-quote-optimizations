#!/bin/bash
# Experiment: Singleton Maps Optimization Impact Measurement
#
# HYPOTHESIS: Optimizing syntax-quote to return singleton maps as map literals
# instead of (apply hash-map (seq (concat ...))) will reduce the size of the 
...
exec "$SHARED_SCRIPT" "singleton-maps" "Singleton Maps Optimization" "$BUILD_SCRIPT" "$RESULTS_DIR"
