...
7. **Validate locally**: AI agents must set up their own environment and run scripts themselves to verify their work and iterate on them until errors are eliminated and the scripts perform as intended. AI agents may stub `strip-nondeterminism` if they are unable to install it to help set up their environment
