#!/bin/bash
# Experiment: Maps Without Splices Optimization Impact Measurement
#
# HYPOTHESIS: Optimizing syntax-quote to use (hash-map k v ...) for maps without
# splices instead of (apply hash-map (seq (concat ...))) will reduce the size of
...
exec "$SHARED_SCRIPT" "maps-without-splices" "Maps Without Splices Optimization" "$BUILD_SCRIPT" "$RESULTS_DIR"
