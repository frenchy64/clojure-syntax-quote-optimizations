#!/bin/bash
# Shared Uberjar Comparison Script
#
# This script compares a baseline Clojure uberjar with an optimized version
# to measure the impact of syntax-quote optimizations.
#
# Usage: ./compare-uberjar.sh <optimization_name> <optimization_title> <build_script> <results_dir>
#
# Parameters:
#   optimization_name: Short name (e.g., "nil", "boolean")
#   optimization_title: Display title (e.g., "Nil Optimization")
#   build_script: Path to the build script for the optimized version
#   results_dir: Directory to store results

set -euo pipefail

if [ $# -lt 4 ]; then
    echo "Usage: $0 <optimization_name> <optimization_title> <build_script> <results_dir>"
    exit 1
fi

OPTIMIZATION_NAME="$1"
OPTIMIZATION_TITLE="$2"
BUILD_SCRIPT="$3"
RESULTS_DIR="$4"
...
echo "=========================================="
echo "Experiment: $OPTIMIZATION_TITLE"
echo "=========================================="
echo ""

# Ensure we're using Java 8 for consistency
echo "Java version:"
...
# Step 2: Build optimized uberjar
echo "=========================================="
echo "Step 2: Build Optimized Uberjar"
echo "=========================================="

echo "Building optimized uberjar with $OPTIMIZATION_NAME optimization..."
...
# Copy the built JAR
if [ ! -f "$TEMP_BUILD_DIR/clojure-${OPTIMIZATION_NAME}-optimized.jar" ]; then
    echo "ERROR: Built JAR not found: $TEMP_BUILD_DIR/clojure-${OPTIMIZATION_NAME}-optimized.jar"
    exit 1
fi

cp "$TEMP_BUILD_DIR/clojure-${OPTIMIZATION_NAME}-optimized.jar" "$RESULTS_DIR/optimized.jar"
...
for class_file in $(find "$RESULTS_DIR/optimized-classes" -name "*.class" -type f); do
    ...
    if [ -z "${ALL_CLASSES[$rel_path]+x}" ]; then
        ALL_CLASSES["$rel_path"]=1
        TOTAL_CLASSES=$((TOTAL_CLASSES + 1))
    fi
done
...
# Step 5: Analyze bytecode differences (LispReader)
echo "=========================================="
...
# Analyze AOT-compiled Clojure classes
echo ""
...
Optimized: Built with $OPTIMIZATION_NAME optimization
...
2. Clojure Compilation Strategy Changes:
   ...
   - These are macros/functions affected by the $OPTIMIZATION_NAME optimization
...
This confirms that:
1. The $OPTIMIZATION_NAME optimization reduces bytecode overhead
2. The change affects $CHANGED_CLOJURE_CLASSES AOT-compiled Clojure classes
3. The optimization has measurable impact on JAR size
EOF
...
This suggests:
1. The optimization adds more bytecode than it saves
...
This could mean:
1. The optimization is used too infrequently to measure
2. Compiler already optimizes effectively
